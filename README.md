...
Fine-tuning은 사전훈련된 모델을 특정 작업에 맞게 조정하는 과정을 말한다. 사전훈련된 모델은 대량의 데이터를 사용해 학습되어, 언어의 기본적인 특성을 이해하게 된다. 그러나 이 모델을 특정 작업에 바로 적용하기에는 제한적일 수 있다. 예를 들어, 감성 분석이나 특정 주제에 대한 문장 분류 등의 작업을 수행하려면 해당 작업에 맞게 모델을 미세 조정해야 한다.

이 때 사용하는 것이 Fine-tuning이다. Fine-tuning은 작업에 맞는 학습 데이터를 사용해 모델의 가중치를 업데이트하고, 출력층을 작업에 맞게 변경하는 과정을 포함항다.
